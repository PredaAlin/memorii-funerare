  ...
  return (
    <section id="plans" className="max-w-6xl mx-auto px-6 py-20 bg-white">
      <div className="text-center mb-16">
        <h2 className="text-4xl font-bold serif text-stone-900 mb-4">Choose Your Legacy</h2>
        <p className="text-stone-500">Select the plan that best honors your loved one.</p>
      </div>
      <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        ...
        <div className="bg-white border-2 border-stone-200 rounded-3xl p-10 shadow-sm hover:shadow-xl transition-all flex flex-col group">
          <div className="mb-8">
            ...
            <h2 className="text-3xl font-bold serif text-stone-800 mt-2">Basic Memorial</h2>
            <p className="text-5xl font-bold text-stone-900 mt-6">${PRICES.basic}</p>
          </div>
          <ul className="space-y-4 mb-10 flex-grow text-sm">
            {['QR Inscribed Steel Plate', '10-Year Data Hosting', '100MB Storage (Photos Only)'].map(f => (
        ...
        <div className="bg-stone-900 border-2 border-amber-600/30 rounded-3xl p-10 shadow-2xl flex flex-col relative overflow-hidden">
          <div className="absolute top-6 right-6 bg-amber-600 text-white text-[10px] font-bold px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg">Recommended</div>
          <div className="mb-8">
            <span className="text-xs font-bold uppercase tracking-widest text-amber-500/60">Professional</span>
            <h2 className="text-3xl font-bold serif text-white mt-2">Premium Legacy</h2>
            <p className="text-5xl font-bold text-white mt-6">${PRICES.premium}</p>
          </div>
          <ul className="space-y-4 mb-10 flex-grow text-sm">
            {['QR Inscribed Steel Plate', 'Guaranteed Lifetime Hosting', '300MB Storage (Photos + Videos)'].map(f => (
