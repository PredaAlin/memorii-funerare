...
**Mobile-first — verify new UI at phone widths (~360px).** Most visitors reach memorial pages by scanning a QR at a graveside, and owners often edit on a phone, so desktop-centric layouts break in practice. Two patterns already fixed (don't regress them): the **Previzualizare modal** caps the `MemorialPreview` phone frame to `max-h-[80vh]`/`max-w-[88vw]` and lays the close button + frame out in a padded, scrollable flex column (a fixed 300×600 frame overflowed phone viewports and made the close button unreachable); the **`MemorialEditor` footer** stacks its buttons full-width on mobile (`flex-col sm:flex-row`, with `order-*` so Save is on top). Avoid fixed-size overlays and `justify-end` single-row button groups that overflow narrow screens.
